...
public class MainFrame_temp extends JFrame {
	...
	private JPanel p, pLog, pTran, pName, pLocation, pIP, pLogStatus,
			pLogButton, pAdress, pHandler, pReceiver, pTranStatus,
			pTranButton;
	private JLabel lblName, lblLocation, lblIP, lblLogStatus, lblAdress, lblHandler, lblReceiver, lblTranStatus;
	private JTextField txtName, txtLocation, txtIP,
	...
	private JPanel pName_1;
	private JLabel LogID;
	private JTextField txtLogId;
	private JPanel pName_2;
	...
	private void initMenu() {
		...
		menuMatch.setHorizontalAlignment(SwingConstants.LEFT);
		menuOperate.add(menuMatch);
		
		mntmNewMenuItem = new JMenuItem("匹配日志数据");
		mntmNewMenuItem.addActionListener(new MatchLogListener());
		mntmNewMenuItem.setFont(new Font("Microsoft YaHei UI", Font.BOLD, 12));
		mntmNewMenuItem.setHorizontalAlignment(SwingConstants.CENTER);
		menuMatch.add(mntmNewMenuItem);
		
		mntmNewMenuItem_1 = new JMenuItem("匹配物流数据");
		mntmNewMenuItem_1.addActionListener(new MatchTransListener());
		mntmNewMenuItem_1.setFont(new Font("Microsoft YaHei UI", Font.BOLD, 12));
		mntmNewMenuItem_1.setHorizontalAlignment(SwingConstants.CENTER);
		menuMatch.add(mntmNewMenuItem_1);
	...
	private void initToolBar() {
		...
		JButton btnMatchTran = new JButton("匹配物流数据");
	...
	private void initLogGatherGUI() {
			...
			pName_1 = new JPanel();
			pLog.add(pName_1);
			pName_1.setLayout(new FlowLayout(FlowLayout.CENTER, 5, 5));
			
			LogID = new JLabel("日志ID：");
			pName_1.add(LogID);
			
			txtLogId = new JTextField();
			txtLogId.setPreferredSize(new Dimension(100, 20));
			pName_1.add(txtLogId);
	...
	private void initTransGatherGUI() {
		...
		pName_2 = new JPanel();
		pTran.add(pName_2);
		pName_2.setLayout(new FlowLayout(FlowLayout.CENTER, 5, 5));
		
		TransID = new JLabel("物流ID：");
		pName_2.add(TransID);
		
		txtTransId = new JTextField();
		txtTransId.setPreferredSize(new Dimension(100, 20));
		pName_2.add(txtTransId);
		...
		pTranStatus.add(comboBox);
		comboBox.addItem("发货中");
		comboBox.addItem("送货中");
		comboBox.addItem("已签收");
	...
	private class GatherTransListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			// 设置物流类型
			int transportType = comboBox.getSelectedIndex() + 1;
			// 将数据包装成物流对象
	...
	private class SaveDataListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			// 保存匹配的物流信息
						...
						//若没有保存成功，则弹出相应的告警提示框
			//保存到文件和数据库
			if(matchedLogs!=null&&matchedTrans!=null)
			{
			...
			{
				JOptionPane.showMessageDialog(null, "匹配的日志数据保存到文件中失败！", "提示",
						JOptionPane.INFORMATION_MESSAGE);
			}
	...
	private void flushMatchedTransTable() {
		//显示物流数据表格
		//创建tableModel,通过标志位区分日志和物流：1,日志；2，物流
				MatchedTableModel tranModel = new MatchedTableModel(transportService.readLogResult(),2);
				//使用tableModel创建JTable
				JTable tranTable = new JTable(tranModel);
				//通过JTable对象创建JScrollPane，显示数据
				JScrollPane scrollPane = new JScrollPane(tranTable);
				//添加日志选项卡
				showPane.addTab("物流",scrollPane);
	}

	//线程类，每隔一分钟刷新一次显示数据表格中的数据，与数据库保持同步
